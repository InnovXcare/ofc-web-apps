...
(function (DocsAPI, window, document, undefined) {
  ...
  DocsAPI.DocEditor = function (placeholderId, config) {
    ...
    _config.editorConfig.canRequestReferenceData =
      _config.events && !!_config.events.onRequestReferenceData;
    _config.editorConfig.canRequestOpen =
      _config.events && !!_config.events.onRequestOpen;
    _config.editorConfig.canRequestSelectDocument =
      _config.events && !!_config.events.onRequestSelectDocument;
    _config.editorConfig.canRequestSelectSpreadsheet =
      _config.events && !!_config.events.onRequestSelectSpreadsheet;
    _config.editorConfig.canRequestReferenceSource =
      _config.events && !!_config.events.onRequestReferenceSource;
    _config.editorConfig.canSaveDocumentToBinary =
      _config.events && !!_config.events.onSaveDocument;
    _config.editorConfig.canStartFilling =
      _config.events && !!_config.events.onRequestStartFilling;
    _config.editorConfig.canRequestRefreshFile =
      _config.events && !!_config.events.onRequestRefreshFile;
    _config.editorConfig.canUpdateVersion =
      _config.events && !!_config.events.onOutdatedVersion;
    _config.editorConfig.canRequestFillingStatus =
      _config.events && !!_config.events.onRequestFillingStatus;
    ...
    var _onMessage = function (msg) {
      if (msg) {
        if (msg.type === "onExternalPluginMessage") {
          ...
          msg.type === "onExternalPluginMessageCallback"
        ) {
          ...
          } else if (msg.event === "onSwitchEditorType" && !handler) {
            if (msg.data) {
              if (typeof msg.data.type == "string")
                localStorage.setItem("asc-force-editor-type", msg.data.type);

              if (msg.data.restart) window.location.reload();
            }
          } else {
    ...
    var _checkConfigParams = function () {
      if (_config.document) {
        ...
        if (
          ...
          console.warn(
            'The "documentType" parameter for the config object must take one of the values word/cell/slide/pdf/diagram.'
          );
        ...
        if (
          ...
          var type =
            /^(?:(xls|xlsx|ods|csv|gsheet|xlsm|xlt|xltm|xltx|fods|ots|xlsb|sxc|et|ett|numbers)|(pps|ppsx|ppt|pptx|odp|gslides|pot|potm|potx|ppsm|pptm|fodp|otp|sxi|dps|dpt|key|odg)|(pdf|djvu|xps|oxps)|(doc|docx|odt|gdoc|txt|rtf|mht|htm|html|mhtml|epub|docm|dot|dotm|dotx|fodt|ott|fb2|xml|oform|docxf|sxw|stw|wps|wpt|pages|hwp|hwpx|md)|(vsdx|vssx|vstx|vsdm|vssm|vstm))$/.exec(
              _config.document.fileType
            );
          if (!type) {
            window.alert(
            ...
            else if (typeof type[3] === "string") _config.documentType = "pdf";
            else if (typeof type[4] === "string") _config.documentType = "word";
            else if (typeof type[5] === "string")
              _config.documentType = "diagram";
          }
        }
    ...
    (function () {
      ...
      if (!!result && result.length && result[1] == "desktop") {
        console.warn(
          "some errors occurred in the desktop app while document opening. please, contact with support team"
        );
      }
    ...
    if (target && _checkConfigParams()) {
      ...
      if (
        _config.editorConfig.customization &&
        _config.editorConfig.customization.integrationMode === "embed"
      )
        window.AscEmbed && window.AscEmbed.initWorker(iframe);

      if (
        _config.document &&
        _config.document.isForm !== true &&
        _config.document.isForm !== false
      ) {
  ...
  function getShardkey(config) {
    ...
    for (var i = scripts.length - 1; i >= 0; i--) {
      if (scripts[i].src.match(/(.*)api\/documents\/api.js/i)) {
        ...
        shardkey &&
          shardkey.length &&
          (config.editorConfig.shardkey = shardkey[1]);
        break;
      }
    }
  }
  ...
  function getAppPath(config) {
    ...
    if (config.document) {
      if (typeof config.document.fileType === "string")
        type =
          /^(?:(pdf)|(djvu|xps|oxps)|(xls|xlsx|ods|csv|xlst|xlsy|gsheet|xlsm|xlt|xltm|xltx|fods|ots|xlsb|numbers)|(pps|ppsx|ppt|pptx|odp|pptt|ppty|gslides|pot|potm|potx|ppsm|pptm|fodp|otp|key|odg)|(oform|docxf)|(vsdx|vssx|vstx|vsdm|vssm|vstm))$/.exec(
            config.document.fileType
          );

      if (config.document.permissions)
        fillForms =
          (config.document.permissions.fillForms === undefined
    ...
    if (type && typeof type[2] === "string") {
      // djvu|xps|oxps
      appType =
        corrected_type === "mobile" || corrected_type === "embedded"
          ? "word"
          : "pdf";
      ...
      if (typeof config.documentType === "string")
        ...
        if (type && typeof type[3] === "string") appType = "cell";
        else if (type && typeof type[4] === "string") appType = "slide";
        else if (type && typeof type[6] === "string") appType = "diagram";
      }
    }
    if (
      !(
        config.editorConfig &&
        config.editorConfig.shardkey &&
        config.document &&
        config.editorConfig.shardkey !== config.document.key
      )
    )
    ...
    const path_type =
      corrected_type === "mobile"
        ? "mobile"
        : corrected_type === "embedded"
        ? fillForms && isForm
          ? "forms"
          : "embed"
        : "main";
    if (appType !== "common") path += "/" + path_type;
    ...
    if (config.editorConfig && path_type !== "forms" && appType !== "common") {
      ...
      if (
        ...
        config.editorConfig.mode === "editole"
      )
  ...
  function getAppParameters(config) {
    ...
    if (config.editorConfig && config.editorConfig.targetApp !== "desktop") {
      ...
      } else params += "&customer={{APP_CUSTOMER_NAME}}";
      if (typeof config.editorConfig.customization == "object") {
        ...
        if (config.editorConfig.customization.logo) {
          if (config.editorConfig.customization.logo.visible === false) {
            ...
            config.editorConfig.customization.logo.imageLight &&
              (params +=
                "&headerlogolight=" +
                encodeURIComponent(
                  config.editorConfig.customization.logo.imageLight
    ...
    var type = config.document
        ? /^(?:(pdf)|(oform|docxf))$/.exec(config.document.fileType)
        : null,
    ...
    if (config.document) {
      ...
      (config.document.isForm === true || config.document.isForm === false) &&
    ...
    if (
      config.editorConfig &&
      config.editorConfig.customization &&
      config.editorConfig.customization.features &&
      config.editorConfig.customization.features.tabBackground
    ) {
      if (
        typeof config.editorConfig.customization.features.tabBackground ===
        "object"
      ) {
        params +=
          "&tabBackground=" +
          (config.editorConfig.customization.features.tabBackground.mode ||
            "header") +
          (config.editorConfig.customization.features.tabBackground.change !==
          false
            ? "-ls"
            : "");
      } else
    ...
    if (
      config.editorConfig &&
      config.editorConfig.customization &&
      config.editorConfig.customization.wordHeadingsColor &&
      typeof config.editorConfig.customization.wordHeadingsColor === "string"
    ) {
      params +=
        "&headingsColor=" +
        config.editorConfig.customization.wordHeadingsColor.replace(/#/, "");
    }
  ...
  function createIframe(config) {
    ...
    iframe.setAttribute(
      "allow",
      "autoplay; camera; microphone; display-capture; clipboard-write;"
    );
  ...
  function extendAppPath(config, path) {
    if (!config.isLocalFile) {
      ...
      if (ver.lastIndexOf("{{") < 0 && path.indexOf(ver) < 0) {
        ...
        if (pos > 0) return [path.slice(0, pos), ver, path.slice(pos)].join("");
      }
    }
    return path;
  }
  ...
  (function () {
    if (document.currentScript) {
      ...
      if (cacheWarmupId && cacheWarmupId.length) {
        DocsAPI.DocEditor.warmUp.call(
          this,
          decodeURIComponent(cacheWarmupId[1])
        );
      }
    }
  })();
