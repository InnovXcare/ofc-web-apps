...
+(function init_themes() {
  ...
  window.uitheme.relevant_theme_id = function () {
    if (this.is_theme_system())
      return this.is_system_theme_dark()
  ...
  !window.uitheme.id &&
    window.uitheme.set_id(localstorage.getItem("ui-theme-id"));
  window.uitheme.iscontentdark =
    localstorage.getItem("content-theme") == "dark";
  ...
  window.uitheme.apply_icons_from_url = function (themeid, url) {
    ...
    const svg_icons_array = [
      base_url + "iconssmall@2.5x.svg",
      base_url + "iconsbig@2.5x.svg",
      base_url + "iconshuge@2.5x.svg",
    ];
    if (window.Common && window.Common.Utils)
      window.Common.Utils.injectSvgIcons(svg_icons_array, true);
    else {
      window.uitheme.svg_icons = [
        base_url + "iconssmall@2.5x.svg",
        base_url + "iconsbig@2.5x.svg",
        base_url + "iconshuge@2.5x.svg",
      ];
  ...
  if (!!objtheme) {
    ...
    if (objtheme) {
      if (window.uitheme.id && window.uitheme.id != objtheme.id) {
        ...
        if (objtheme.colors) {
          ...
          // Also inject as :root styles for immediate application
          let rootColors = [];
          for (let c in objtheme.colors) {
        ...
        if (objtheme.skeleton) {
          if (objtheme.skeleton.css) inject_style_tag(objtheme.skeleton.css);
    ...
    if (
      window.uitheme.id &&
      window.uitheme.id.lastIndexOf("theme-gray", 0) === 0
    ) {
      header_tokens.forEach(function (i) {
      ...
      window.uitheme.id &&
      window.uitheme.id.lastIndexOf("theme-white", 0) === 0
    ) {
      header_tokens.forEach(function (i) {
