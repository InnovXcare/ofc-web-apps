  ...
  Common.UI.Themes = new (function (locale) {
    ...
    var parse_themes_object = function (obj) {
      ...
      if (!!obj.themes && obj.themes instanceof Array) {
        ...
        theme_label =
          !obj.l10n || !obj.l10n[curr_lang] ? obj.name : obj.l10n[curr_lang];
    ...
    var get_themes_config = function (url) {
      ...
      Common.Utils.loadConfig(url, function (obj) {
        if (obj != "error") {
          ...
          check_launched_custom_theme.call(me);
        } else {
    ...
    var on_document_ready = function (el) {
      check_launched_custom_theme.call(this);

      if (
        !Common.Controllers.Desktop.isActive() /*|| !Common.Controllers.Desktop.isOffline()*/
      ) {
        // get_themes_config("../../common/main/resources/themes/themes.json");

        get_themes_config.call(this, "../../../../themes.json");
      } else if (Common.Controllers.Desktop.localThemes()) {
        parse_themes_object({
          themes: Common.Controllers.Desktop.localThemes(),
        });
      }
    };
    ...
    var get_ui_theme_name = function (objtheme) {
      ...
      if (objtheme && typeof objtheme == "object") return objtheme.id;
    ...
    const get_system_default_theme = function () {
      const id =
        get_system_theme_type() == THEME_TYPE_DARK
          ? window.uitheme.DEFAULT_DARK_THEME_ID
    ...
    const refresh_theme = function (force, caller) {
      if (
        force ||
        Common.localStorage.getItem("ui-theme-id") != window.uitheme.id
      ) {
    ...
    return {
      init: function (api) {
        ...
        if (themes_map[theme_id]) {
          if (themes_map[theme_id].src && themes_map[theme_id].src.colors) {
            // Theme loaded from JSON - inject its CSS
            write_theme_css(
        ...
        // Ensure body has theme classes BEFORE reading computed colors
        if (!document.body.classList.contains(theme_id)) {
        ...
        const is_content_dark =
          themes_map[theme_id] &&
          themes_map[theme_id].type == "dark" &&
        ...
        if (themes_map[theme_id] && themes_map[theme_id].icons) {
          if (
            !document.querySelector("style#" + theme_id) &&
            !(window.uitheme.embedicons === true)
          ) {
            const icons_base_url = !!themes_map[theme_id].icons.basepath
              ? themes_map[theme_id].icons.basepath
        ...
        if (
          !(Common.Utils.isIE10 || Common.Utils.isIE11) &&
          !Common.Controllers.Desktop.isActive()
        )
          window
      ...
      map: function () {
        if (
          Common.Controllers.Desktop.isActive() &&
          !Common.Controllers.Desktop.systemThemeSupported()
        ) {
      ...
      currentThemeId: function () {
        return !!themes_map[window.uitheme.id]
          ? window.uitheme.id
          : window.uitheme.DEFAULT_LIGHT_THEME_ID;
      },
      ...
      isDarkTheme: function (id) {
        ...
        return (
          (is_theme_type_system(id)
            ...
            : themes_map[id].type) == THEME_TYPE_DARK
        );
      },
      ...
      setContentTheme: function (mode, force, keep) {
        ...
        if (set_dark != window.uitheme.iscontentdark || force) {
          ...
          if (this.isDarkTheme()) this.api.asc_setContentDarkMode(set_dark);

          if (
            !(keep === false) &&
            Common.localStorage.getItem("content-theme") != mode
          )
      ...
      getThemeProps: function (prop) {
        if (prop === "font") {
          ...
          return Common.Utils.isIE ||
            ...
            : window
                .getComputedStyle(document.body)
                .getPropertyValue("--toolbar-preferred-tab-style") || "line";
        }
